...
pub fn build<'a>() -> App<'static, 'a> {
    SubCommand::with_name("gzip")
        .about("Create a tar file with gzip compression")
        .arg(
            Arg::with_name("file")
                .short("f")
                .long("file")
    ...
    Operation::Create(
        {
            ...
            match Comp::from_format(Format::Gzip(get_comp_level(x)), w) {
                Ok(x) => x,
                Err(e) => {
                    println!("Building gzip compressor failed");
                    println!("{:?}", e);
                    ::std::process::exit(1);
                }
            }
