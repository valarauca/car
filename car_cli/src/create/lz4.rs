...
pub fn build<'a>() -> App<'static, 'a> {
    SubCommand::with_name("lz4")
        .about("Create a tar file with lz4 compression")
        .arg(
            Arg::with_name("file")
                .short("f")
                .long("file")
    ...
    Operation::Create(
        {
            ...
            match Comp::from_format(Format::Lz4(get_comp_level(x)), w) {
                Ok(x) => x,
                Err(e) => {
                    println!("Building lz4 compressor failed");
                    println!("{:?}", e);
                    ::std::process::exit(1);
                }
            }
