...
pub fn build<'a>() -> App<'static, 'a> {
    SubCommand::with_name("xz")
        .about("Create a tar file with xz compression")
        .arg(
            Arg::with_name("file")
                .short("f")
                .long("file")
    ...
    Operation::Create(
        {
            ...
            match Comp::from_format(Format::Xz(get_comp_level(x)), w) {
                Ok(x) => x,
                Err(e) => {
                    println!("Building xz compressor failed");
                    println!("{:?}", e);
                    ::std::process::exit(1);
                }
            }
