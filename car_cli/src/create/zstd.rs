...
pub fn build<'a>() -> App<'static, 'a> {
    SubCommand::with_name("zstd")
        .about("Create a tar file with zstd compression")
        .arg(
            Arg::with_name("file")
                .short("f")
                .long("file")
    ...
    Operation::Create(
        {
            ...
            match Comp::from_format(Format::Zstd(get_comp_level(x)), w) {
                Ok(x) => x,
                Err(e) => {
                    println!("Building zstd compressor failed");
                    println!("{:?}", e);
                    ::std::process::exit(1);
                }
            }
