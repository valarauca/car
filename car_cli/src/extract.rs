...
fn build_path<P: AsRef<Path>>(path: P) -> io::Result<File> {
    ...
    loop {
        ...
        match (&curr,&next) {
            ...
            (&Option::None, _) => return Err(Error::new(ErrorKind::InvalidData, "Path has no length")),
            (&Option::Some(Component::Prefix(_)),_) |
            (_,&Option::Some(&Component::Prefix(_))) |
            (&Option::Some(Component::RootDir),_) |
            (_,&Option::Some(&Component::RootDir)) => return Err(Error::new(ErrorKind::InvalidData, "Must be relative path")),
            (&Option::Some(Component::ParentDir),_)|
            (_,&Option::Some(&Component::ParentDir)) => return Err(Error::new(ErrorKind::InvalidData, "URHAXSUX")),
